...
(use-package centaur-tabs
  ...
  (setq centaur-tabs-mouse-close-tab-function #'kill-buffer
        ;; Prevent aggressive window management
        centaur-tabs-adjust-buffer-order nil)

  ;; Mouse support configuration - add manual mouse bindings
  (defun my/centaur-tabs-mouse-close (event)
    ...
    (when (display-graphic-p)
      ...
      (let* ((posn (event-start event))
             ...
             (tab-buffer (get-text-property (posn-point posn) 'centaur-tabs)))
        (cond
         ;; If we clicked on a specific tab, close that buffer
         (tab-buffer
          (let ((tab-buffer-name (buffer-name tab-buffer)))
            (message "Attempting to close tab buffer: %s" tab-buffer-name)
            (kill-buffer tab-buffer-name)))
         ;; Otherwise, close current buffer in window
         (window
  ...
  ;; Set up mouse bindings globally for the tab area
  (when (display-graphic-p)
    (global-set-key [tab-line mouse-2] #'my/centaur-tabs-mouse-close)
