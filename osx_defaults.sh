...
#
# Author: @jbellingham91
# Thanks to: @erikh, @DAddYE, @mathiasbynens, @vraravam
#
...
sudo defaults write -g ApplePressAndHoldEnabled 0
...
if ask "Add a message to the login screen" Y; then
  sudo defaults write /Library/Preferences/com.apple.loginwindow LoginwindowText "Beware\! You are logging into Jesse's laptop\!"
fi
...
if ask "Set computer name (as done via System Preferences → Sharing)" Y; then
  sudo scutil --set ComputerName "Jesse's MacBook Pro"
...
if ask "Set the icon size of Dock items to 65 pixels" Y; then
...
if ask "Zoomus settings" Y; then
  ...
  defaults write ZoomChat ZoomEnterFullscreenWhenDualMonitorSetted -bool true
  defaults write ZoomChat ZoomFitDock -bool true
  defaults write ZoomChat ZoomFitXPos -int 2583
  defaults write ZoomChat ZoomFitYPos -int 1476
