tap "homebrew/bundle"
tap "homebrew/cask-fonts"
tap "homebrew/services"
tap "jandedobbeleer/oh-my-posh"
tap "jez/formulae"
tap "localstack/tap"
tap "ykursadkaya/openlens"
brew "act"
brew "awscli"
brew "azure-cli"
brew "bash"
brew "bat"
brew "btop"
brew "cocoapods"
brew "direnv"
brew "docker-compose"
brew "docker-credential-helper"
brew "eza"
brew "fswatch"
brew "fzf"
brew "gcc"
brew "git"
brew "git-delta"
brew "git-extras"
brew "jesseduffield/lazygit/lazygit"
brew "jq"
brew "jump"
brew "lima"
brew "mas"
brew "mise"
brew "mcfly"
brew "postgresql@16"
brew "pre-commit"
brew "ripgrep"
brew "sbt"
brew "talisman"
brew "telnet"
brew "tldr"
brew "tmux"
brew "trash", link: true
brew "watch"
brew "watchman"
brew "wget"
brew "yamllint"
brew "yq"
brew "zsh"
brew "zsh-autosuggestions"
brew "zsh-syntax-highlighting"
brew "jandedobbeleer/oh-my-posh/oh-my-posh"
brew "localstack/tap/localstack-cli"
cask "1password" if OS.mac?
cask "1password-cli" if OS.mac?
cask "adobe-acrobat-reader" if OS.mac?
cask "cheatsheet" if OS.mac?
cask "devtoys" if OS.mac?
cask "discord" if OS.mac?
cask "docker-desktop" if OS.mac?
cask "firefox" if OS.mac?
cask "flux-app" if OS.mac?
cask "flycut" if OS.mac?
cask "font-hack-nerd-font" if OS.mac?
cask "font-jetbrains-mono-nerd-font" if OS.mac?
cask "font-monaspace" if OS.mac?
cask "font-sf-mono" if OS.mac?
cask "font-sf-pro" if OS.mac?
cask "ghostty" if OS.mac?
cask "google-chrome" if OS.mac?
cask "jetbrains-toolbox" if OS.mac?
cask "logi-options+" if OS.mac?
cask "meetingbar" if OS.mac?
cask "microsoft-edge" if OS.mac?
cask "microsoft-onenote" if OS.mac?
cask "notion" if OS.mac?
cask "raycast" if OS.mac?
cask "rectangle" if OS.mac?
cask "sf-symbols" if OS.mac?
cask "spotify" if OS.mac?
cask "stats" if OS.mac?
cask "visual-studio-code" if OS.mac?
cask "wakatime" if OS.mac?
cask "warp" if OS.mac?
cask "wezterm" if OS.mac?
cask "yubico-authenticator" if OS.mac?
cask "zen" if OS.mac?
mas "1Password for Safari", id: 1569813296 if OS.mac?
mas "HP Smart", id: 1474276998 if OS.mac?
mas "Keynote", id: 409183694 if OS.mac?
mas "Numbers", id: 409203825 if OS.mac?
mas "Pages", id: 409201541 if OS.mac?
mas "Xcode", id: 497799835 if OS.mac?
mas "Refined GitHub", id: 1519867270 if OS.mac?
mas "Raycast", id: 1450147040 if OS.mac?
mas "Microsoft OneNote", id: 784801555 if OS.mac?
# mas "Amphetamine", id: 937984704 if OS.mac?
# mas "Amphetamine Expired", id: 1444383602 if OS.mac?
# mas "Amphetamine Menu Bar", id: 1444383601 if OS.mac?
# mas "Amphetamine Preferences", id: 1444383600 if OS.mac?
# mas "Amphetamine Widgets", id: 1444383603 if OS.mac?

# VSCode Extensions
vscode "adammaras.vscode-ansible"
vscode "adammaras.vscode-ansible-lint"
vscode "adpyke.vscode-sql-formatter"
vscode "bbenoist.nix"
vscode "christian-kohler.npm-intellisense"
vscode "chrmarti.regex"
vscode "dbaeumer.vscode-eslint"
vscode "esbenp.prettier-vscode"
vscode "fabiospampinato.vscode-diff"
vscode "felipecaputo.git-project-manager"
vscode "fnando.linter"
vscode "formulahendry.auto-rename-tag"
vscode "github.copilot"
vscode "github.copilot-chat"
vscode "golang.go"
vscode "huizhou.githd"
vscode "infeng.vscode-react-typescript"
vscode "janisdd.vscode-edit-csv"
vscode "jnoortheen.nix-ide"
vscode "kisstkondoros.vscode-codemetrics"
vscode "mechatroner.rainbow-csv"
vscode "miclo.sort-typescript-imports"
vscode "ms-azuretools.vscode-docker"
vscode "ms-python.debugpy"
vscode "ms-python.python"
vscode "ms-python.vscode-pylance"
vscode "ms-vscode-remote.remote-containers"
vscode "ms-vsliveshare.vsliveshare"
vscode "nicoespeon.abracadabra"
vscode "orta.vscode-jest"
vscode "pflannery.vscode-versionlens"
vscode "pmneo.tsimporter"
vscode "pnp.polacode"
vscode "prisma.prisma"
vscode "redhat.vscode-xml"
vscode "redhat.vscode-yaml"
vscode "ritwickdey.liveserver"
vscode "rubymaniac.vscode-direnv"
vscode "rvest.vs-code-prettier-eslint"
vscode "svelte.svelte-vscode"
vscode "vincaslt.highlight-matching-tag"
vscode "vscjava.vscode-java-pack"
vscode "vscodevim.vim"
vscode "wakatime.vscode-wakatime"
vscode "wix.vscode-import-cost"
vscode "xabikos.javascriptsnippets"
vscode "yzhang.markdown-all-in-one"
